---
title: "CGE in R platform"
subtitle: "Practical sessions - Supporting materials "
author: 
  - Frédéric Reynès 
  - Anissa Saumtally
...
Running your first simulation and getting the results with the one equation model
presenting the datafull output database 


# Introduction 

## Foreword: codename *Ermeeth*
...
## Basic folder strucure presentation 
...
### Step 3: Produce selected result templates (**OPTIONAL**)
...
library(tidyverse)
# data_full <- readRDS("sampledata/standard_shocks.rds") |> filter(variable %in% c("GDP","LF","PVA", "PCH", "PY", "PX", "DISPINC_AT","EMS","I","X","W","CH","PM","M","I_SNF","PYM","PCI","C_L","F_L_Q","F_L_NQ","VA_SM","DISPINC_AT"))
# saveRDS(data_full, file = "standard_shocks_lite.rds")
...
-   Look into the data_full database (you can do this in the `Main_one_eq.R` file)
    ...
    -   Create a new data.frame that contains the GDP (`Y`) growth rate for the baseline and the shock scenario.
...
-   Create a quarto `.qmd` file that :
    ...
    -   computes the GDP growth rate for the baseline and shock scenarios
...
## `mdl` calib data files

Add example of failure to calibrate properly
